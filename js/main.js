...
//Функция проверки полей ввода сотрудников на русские буквы
function inputEmployersCheck () {
//Перебор массива и проверка инпутов после нажатия и отпускания кнопки
		for (let i = 0; i < hireEmployersItem.length; i++) {
			hireEmployersItem[i].addEventListener('keyup', () => {
				let pattern = /^[А-ЯЁ][а-яё]*$/i;//Регулярное выражение для кириллицы
			//Проверка вводимых данных на соответствие условию регулярного выражения
			if (pattern.test(hireEmployersItem[i].value) || hireEmployersItem[i].value === '') {
			} else {
				alert('Введите имена сотрудников русскими буквами!');
			}		
		});
	}
};
